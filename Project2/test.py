...
def prepare_vao_obj(obj):
    ...
    arr = []
    normal_num = len(obj.normal_vector)
    vertex_num = len(obj.vertex_position)

    for i in range(0, vertex_num):
        # vertex
        arr.append(obj.vertex_position[i][0])
        arr.append(obj.vertex_position[i][1])
        arr.append(obj.vertex_position[i][2])
        ...
        # normal
        if i < normal_num:
            arr.append(obj.normal_vector[i][0])
            arr.append(obj.normal_vector[i][1])
            arr.append(obj.normal_vector[i][2])

    # for i in range(0, len(obj.vertex_position)):
    #     varr.extend(obj.vertex_position[i])

    for i in range(0, len(obj.normal_index)):
        niarr.extend(obj.normal_index[i])

    for i in range(0, len(obj.vertex_index)):
        viarr.extend(obj.vertex_index[i])
    ...
    ver = glm.array(glm.float32, *arr)
    v_indices = glm.array(glm.uint32, *viarr)
    n_indices = glm.array(glm.uint32, *niarr)
    ...
    # create and activate VBO (vertex buffer object)
    VBO_V = glGenBuffers(1)   # create a buffer object ID and store it to VBO variable
    glBindBuffer(GL_ARRAY_BUFFER, VBO_V)  # activate VBO as a vertex buffer object

    # vertex index
    EBO_V = glGenBuffers(1)
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO_V)

    EBO_N = glGenBuffers(1)
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO_N)

    glBufferData(GL_ARRAY_BUFFER, ver.nbytes, ver.ptr, GL_STATIC_DRAW) # allocate GPU memory for and copy vertex data to the currently bound vertex buffer
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, v_indices.nbytes, v_indices.ptr, GL_STATIC_DRAW)
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, n_indices.nbytes, n_indices.ptr, GL_STATIC_DRAW)
    ...
    #     for i in range(0, len(obj.normal_vector)):
    #         narr.extend(obj.normal_vector[i])
    #     for i in range(0, len(obj.normal_index)):
    #         niarr.extend(obj.normal_index[i])

    #     normals = glm.array(glm.float32, *narr)
    #     n_indices = glm.array(glm.uint32, *niarr)
    ...
    #     glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 3 * glm.sizeof(glm.float32), ctypes.c_void_p(6 * len(obj.vertex_position) * glm.sizeof(glm.float32)))
