...
def main():
    ...
    vao_triangle_x = prepare_vao_triangle()
    vao_triangle_y = prepare_vao_triangle()
    ...
    while not glfwWindowShouldClose(window):
        ...
        # update uniform for triangle x
        M = XT
        glUniformMatrix3fv(M_loc, 1, GL_TRUE, M)
        glBindVertexArray(vao_triangle_x)
        glDrawArrays(GL_TRIANGLES, 0, 3)

        # update uniform for triangle y
        M = YT
        glUniformMatrix3fv(M_loc, 1, GL_TRUE, M)
        glBindVertexArray(vao_triangle_y)
        glDrawArrays(GL_TRIANGLES, 0, 3)

        # update uniform for white line
        M = R
